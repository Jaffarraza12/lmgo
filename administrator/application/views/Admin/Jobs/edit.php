...
<div class="span12">
    ...
    <!-- BEGIN FORM-->
    <form action="EditJob" method="POST" class="form-horizontal" enctype="multipart/form-data">

        <?php $this->load->view("Admin/common/breadcrumb.php"); ?>
        <input name="url" class="url" type="hidden" value="<?php echo base_url() ?>" />
        <input name="job_id"  type="hidden" value="<?php echo $jobinfo->job_id ?>" />
                ...
                <div class="portlet box <?php echo $row->color; ?>">
                    ...
                    <div class="portlet-body">
                        ...
                        <div class="row-fluid">
                            ...
                            <div class="controls">
                                <input type="text" name="title_<?php echo $langCode; ?>" value="<?php echo $jobinfo->title ?>" placeholder="Please enter title" class="m-wrap span6 popovers" data-original-title="Title" data-content="Please enter title" data-trigger="hover"  />
                            ...
                            <div class="controls">
                                <select name="job_type_id" placeholder="Please select status" class="m-wrap popovers" data-original-title="Status" data-content="Please select Job Type" data-trigger="hover"  >
                                    <?php foreach($jobtypes as $jobtype) {
                                        if($jobtype->job_type_id == $jobinfo->job_type_id ) {
                                          echo  '<option selected="selected" value="'. $jobtype->job_type_id.'">'. $jobtype->name.'</option>';
                                        } else {
                                          echo  '<option  value="'. $jobtype->job_type_id.'">'. $jobtype->name.'</option>'; }
                                        }
                                    ?>
                                </select>
                            </div>
                            ...
                            <div class="controls">
                                <select name="category_id" placeholder="Please select status" class="m-wrap popovers" data-original-title="Status" data-content="Please select Employer" data-trigger="hover"  >
                                    <?php foreach($categories as $category) {
                                        if($category->category_id == $jobinfo->category_id ) {
                                          echo '<option selected="selected" value="'.$category->category_id.'">'.$category->name.'</option>';
                                        } else {
                            ...
                            <div class="controls">
                                <select name="employer_id" placeholder="Please select status" class="m-wrap popovers" data-original-title="Status" data-content="Please select Employer" data-trigger="hover"  >
                                    <?php foreach($employers as $employer) {
                                         if($employer->employer_id == $jobinfo->employer_id){
                                             echo  '<option selected="selected" value="'.$employer->employer_id.'">'.$employer->account_name.'</option>';
                                         } else {
                                             echo  '<option value="' . $employer->employer_id . '">' . $employer->name . '</option>';
                                         }
                                     }  ?>
                                </select>
                            </div>
                            ...
                            <div class="controls">
                                <select name="country_id" placeholder="Please select status" class="m-wrap popovers" data-original-title="Status" data-content="Please select Country" data-trigger="hover" onchange="Myjava.GetCities(this.value);"  >
                                    <?php foreach($countries as $country) {
                                        if($country->country_id == $jobinfo->country_id){
                                           echo '<option selected="selected" value="'. $country->country_id.'">'. $country->en_title.'</option>';
                                        } else {
                                           echo '<option value="'. $country->country_id.'">'. $country->en_title.'</option>';
                                        }
                                    }  ?>
                                </select>
                            </div>
                            ...
                            <div class="controls">
                                <select name="city_id" id="city_id" placeholder="Please select status" class="m-wrap popovers" data-original-title="Status" data-content="Please select Cities" data-trigger="hover">
                                    <?php foreach($cities as $city):
                                        echo '<option value="'.$city->city_id.'">'.$city->en_title.'</option>';
                                    endforeach;?>
                        ...
                        <div class="row-fluid">
                            ...
                            <div class="control-group">
                                ...
                                <div class="controls">
                                    <textarea id="ckeditor_<?php echo $langCode; ?>" class="span12 ckeditor m-wrap" name="editor_<?php echo $langCode; ?>" rows="6"><?php echo $jobinfo->description ?></textarea>
                                </div>
                            </div>
...
<div class="portlet box blue map" id="secMap" >
    ...
    <div class="portlet-body">
        ...
        <input type="hidden" id="txtLatitude" name="latitude" value="<?php echo $jobinfo->latitude ?>" />
        <input type="hidden" id="txtLongitude" name="longitude" value="<?php echo $jobinfo->longitube ?>" />
        ...
        <?php foreach($attributes as $attribute) { ?>
            <div class="control-group">
                ...
                <?php if($attribute->type == 'TEXTBOX') { ?>
                    <div class="controls">
                        <input type="text" value="<?php echo $attribute_info[$attribute->attribute_id]['value'] ?>" name="attribute[<?php echo $attribute->attribute_id ?>]" size="16" class="m-wrap m-ctrl-medium">
                    </div>
                    ...
                    <div class="controls">
                        <select name="attribute[<?php echo $attribute->attribute_id ?>]" placeholder="Please select status" class="m-wrap popovers" data-original-title="Status" data-content="Please select attribute Option" data-trigger="hover">
                            <?php foreach($attribute_option[$attribute->attribute_id] as $option) {
                                if($option['attribute_option_id'] == $attribute_info[$attribute->attribute_id]['attribute_option_id'] ){
                                    echo '<option selected="selected" value="'.$option['attribute_option_id'].'">'.$option['name'].'</option>';
                                } else {
                                    echo '<option value="'.$option['attribute_option_id'].'">'.$option['name'].'</option>';
                                }
                            } ?>
                        </select>
                    </div>
