...
class TicketMessage extends Action
{
    ...
    function AppendPostFile($_postKey,$_userId)
    {
        if(!empty($_FILES[$_postKey]) && true)
        {
            ...
            {
                KnowledgeBase::CreateEntry("SYSTEM", $fileId, $fileurid, 3, $filename, 0, 100);
...
class Ticket extends Action
{
    ...
    {
        $result = DBManager::Execute(true, "SELECT * FROM `" . DB_PREFIX . DATABASE_TICKETS . "` WHERE `user_id`='" . DBManager::RealEscape($_id) . "' AND `last_update` > ".intval($_newerThan)." ORDER BY `last_update` DESC LIMIT 1;");
        if($result)
            if($row = DBManager::FetchArray($result))
            {
