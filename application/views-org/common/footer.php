...
<div class="footer-cta">
				<div class="container">
					...
					<p>We're here to help. Check out our FAQs, send us an email or call us at 921111</p>
						...
						<div class="row">
							<aside id="text-2" class="footer-widget widget_text col-md-3 col-sm-6 col-xs-12">
								<div class="textwidget">
									...
									<form id="pmc_mailchimp" method="post">
										<div>
											<label for="pmc_email">Enter your email</label><br/>
											<input name="pmc_email" id="pmc_email" type="email" placeholder="Email Address" required="required"/>
										</div>
										<div>	
											<div id="loading" style="border: 0px solid red; position: absolute; float: left; width: 126px; height: 52px; margin: 0px 0px; text-align: center; background: #fff; opacity: 0.9; line-height: 55px; display: none;">
												<i class="fa fa-refresh fa-spin"></i>
											</div>
											<?php if( !empty($login) ) : ?>										
											<input type="button" value="Sign Up" onclick="newsletter();" style="background: orange; height: 52px; width: 126px;"/>
											<?php else : ?>
											<input type="button" value="Sign Up" onclick="newsletter();" style="background: #01da90; height: 52px; width: 126px;"/>
											<?php endif; ?>
										</div>
							...
							<aside id="pages-2" class="footer-widget widget_pages col-md-3 col-sm-6 col-xs-12">
								...
								<?php 
								$sitemap = $this->db->query("SELECT * FROM content 
															INNER JOIN content_data ON content_data.contentid
															WHERE content.contentid = content_data.contentid AND content.type='Sitemap' AND content.status='1'")->result();								
								
								?>
								<ul>
									<?php foreach( $sitemap as $href ): ?>
									<li class="page_item page-item-1719"><a href="<?php echo base_url('index.php/common/information/index/'.$href->tag); ?>"><?php echo $href->title; ?></a></li>
									<?php endforeach; ?>
								</ul>
							</aside>
							<aside id="recent-posts-2" class="footer-widget widget_recent_entries col-md-3 col-sm-6 col-xs-12">
								...
								<?php 
								$article = $this->db->query("SELECT * FROM content INNER JOIN content_data ON   content.contentid = content_data.contentid WHERE type='Articles' AND status='1' ORDER BY content.contentid DESC LIMIT 3")->result();
								
								?>
								<ul>
									<?php foreach( $article as $art ): ?>
									<li>
										<a href="<?php echo base_url('index.php/common/information/index/'.$art->tag); ?>"><?php echo $art->title; ?></a>
...
function newsletter()
{
	...
	else if(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(pmc_email)!==true)
	{
	...
	$.ajax({
        type: 'POST',
        url: '<?php echo base_url()?>index.php/common/home/newsletter',
        data: { pmc_email:pmc_email },
        success:function(response){

            if(response == 'Newsletter Subscription successfully') {
                var msg = '<div class="alert alert-success" role="alert" style="width:auto;">Subscription successfully</div>'
            }
            else if(response == 'Already Subscribed for newsletter') {
            	var msg = '<div class="alert alert-danger" role="alert" style="width:auto;">Already Subscribed for newsletter</div>'
            }
           $("#msg").html(msg);
           $('#loading').hide();
        }
...
<script src="<?php echo base_url(); ?>assets/javascript/wp-content,_plugins,_woocommerce,_assets,_js,_frontend,_cart-fragments.min.js,qver==2.2.11+wp-includes,_js,_comment-reply.min.js,qver==4.1.1.pagespeed.jc.bizookD4xW.js"></script>
